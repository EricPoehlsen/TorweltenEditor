...
class ImageScreen(tk.Frame):
    ...
    def __init__(self,main,app):
        ...
        self.variant_frame = tk.LabelFrame(self.edit_frame,text = msg.IS_SET_SELECTION)
    ...
    def imageError(self,filename):
        ...
        message.config(font = "Arial 10 bold",state = tk.DISABLED, background = "#eeeeee")
        message.grid(row = 0, column = 1)
        button = tk.Button(window,text = msg.IS_ERROR_CLOSE,command = window.destroy)
    ...
    def _displayImage(self):
        if self.image is not None:
            ...
            resized = self.image.resize((int(real_width*scale),int(real_height*scale))) # ,resample=Image.ANTIALIAS)
            tk_image = ImageTk.PhotoImage(image = resized)
            self.images.append(tk_image)
            self.img_on_canvas = self.image_canvas.create_image(int(canvas_dim/2),int(canvas_dim/2), image = tk_image)
            
            self._showAspectButtons()


    def _aspectRatios(self):
        aspect = {}
        aspect[page.SINGLE] = 1.0 * page.SINGLE_WIDTH/page.SINGLE_HEIGHT
        aspect[page.WIDE] = 1.0 * page.DOUBLE_WIDTH/page.SINGLE_HEIGHT
        aspect[page.DOUBLE] = 1.0 * page.SINGLE_WIDTH/page.DOUBLE_HEIGHT
        aspect[page.QUART] = 1.0 * page.DOUBLE_WIDTH/page.DOUBLE_HEIGHT
        aspect[page.TRIPLE] = 1.0 * page.SINGLE_WIDTH/page.TRIPLE_HEIGHT
        aspect[page.BIG] = 1.0 * page.DOUBLE_WIDTH/page.TRIPLE_HEIGHT
        aspect[page.FULL] = 1.0 * page.SINGLE_WIDTH/page.FULL_HEIGHT
        aspect[page.HALF] = 1.0 * page.DOUBLE_WIDTH/page.FULL_HEIGHT
        aspect[page.ATTRIB_IMAGE] = 0.805464 # every code needs magic numbers :P
    ...
    def _showAspectButtons(self):
        variants = []
        variants.append((page.ATTRIB_IMAGE,msg.PDF_ATTRIB_IMAGE))
        variants.append((page.SINGLE,msg.PDF_SINGLE))
        variants.append((page.WIDE,msg.PDF_WIDE))
        variants.append((page.DOUBLE,msg.PDF_DOUBLE))
        variants.append((page.QUART,msg.PDF_QUART))
        variants.append((page.TRIPLE,msg.PDF_TRIPLE))
        variants.append((page.BIG,msg.PDF_BIG))
        variants.append((page.FULL,msg.PDF_FULL))
        variants.append((page.HALF,msg.PDF_HALF))

        for variant in variants:
            button = tk.Button(self.variant_frame,text = variant[1],anchor=tk.W)
        ...
        if self.rect is None:
            ...
            if image_tag is not None:
                ...
                if size_tag is not None:
                    ...
                    while x1 + box_width >= width: box_width -= 1
                ...
                # draw selection box ... 
                self.rect = self.image_canvas.create_rectangle(x1,y1,x2,y2,fill = "", outline = "#ffff00",width = border)
    ...
    def _moveFrame(self,event):
        ...
        if self.rect is not None:
            ...
            if dx < 0 and abs(dx) > left_margin: dx = -left_margin
            if dx > right_margin: dx = right_margin
            if dy < 0 and abs(dx) > top_margin <=5: dy = -top_margin
    ...
    def _scaleFrame(self,event):
        ...
        if self.rect is not None:
            ...
            if event.char == "+":
                if rect_width < 0.99 * image_width and rect_height < 0.99 * image_height:
                    self.image_canvas.scale(self.rect,scale_center_x,scale_center_y,1.01,1.01) 
                    if left_margin <= 5: self.image_canvas.move(self.rect,1,0)
                    if right_margin <= 5: self.image_canvas.move(self.rect,-1,0)
                    if top_margin <= 5: self.image_canvas.move(self.rect,0,1)
                    if bottom_margin <= 5: self.image_canvas.move(self.rect,0,-1)

            if event.char == "-":
                self.image_canvas.scale(self.rect,scale_center_x,scale_center_y,0.99,0.99) 
    ...
    def _updateBoundingBox(self):
        ...
        if self.finished == 1:
            if self.rect is not None:
                ...
                self.char.setImageSize(self.selected_size,x,y,scale)
                self.image_canvas.itemconfig(self.rect,outline = "#00dd00")


    # this defines the load / remove button for an image ... 
    def _checkForImage(self):
        """ check if an image file is defined in the character xml and set the button for
        importing or deleting an image """

        image_tag = self.char.getImage()
        if image_tag is not None:
            self.load_button.config(text = msg.IS_REMOVE_IMAGE, command = self._removeImage)
        else:
            self.load_button.config(text = msg.IS_IMPORT_IMAGE, command = self.loadImageWindow)
