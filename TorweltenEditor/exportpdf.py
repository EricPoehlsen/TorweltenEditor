...
class ExportPdf:
    ...
    def renderPage(self,canvas,page):
        if page is not None:
            ...
            for module in modules: 
                ...
                if mod_type == page_data.MOD_ATTRIBUTES: self.moduleAttributes(**kwargs)
                elif mod_type == page_data.MOD_TRAITS: self.moduleTraits(**kwargs)
                elif mod_type == page_data.MOD_SKILLS: self.moduleSkills(**kwargs)
                elif mod_type == page_data.MOD_WEAPONS: self.moduleWeapons(**kwargs)
                elif mod_type == page_data.MOD_EQUIPMENT: self.moduleEquipment(**kwargs)
                elif mod_type == page_data.MOD_CONTACTS: self.moduleContacts(**kwargs)
                elif mod_type == page_data.MOD_NOTES: self.moduleNotes(**kwargs)
                elif mod_type == page_data.MOD_EWT: self.moduleEWT(**kwargs)
                elif mod_type == page_data.MOD_IMAGE: self.moduleImage(**kwargs)

    # standard one page sheet ... 
    def renderStandard(self):
        self.moduleAttributes(self.pdf,X_ORIGINS[0],Y_ORIGINS[0],DOUBLE)
        self.moduleTraits(self.pdf,X_ORIGINS[1],Y_ORIGINS[0],DOUBLE)
        self.moduleSkills(self.pdf,X_ORIGINS[0],Y_ORIGINS[2],DOUBLE,"active")
        self.moduleSkills(self.pdf,X_ORIGINS[1],Y_ORIGINS[2],DOUBLE,"passive")
        self.moduleWeapons(self.pdf,X_ORIGINS[2],Y_ORIGINS[0],size = DOUBLE,variant = "guns", info_lines = 3)
        self.moduleWeapons(self.pdf,X_ORIGINS[2],Y_ORIGINS[2],variant = "melee", info_lines = 1)
        self.moduleEWT(self.pdf,X_ORIGINS[2],Y_ORIGINS[3])
        self.moduleEquipment(self.pdf,X_ORIGINS[3],Y_ORIGINS[0],size = DOUBLE,condensed = True)
        self.moduleContacts(self.pdf,X_ORIGINS[3],Y_ORIGINS[2],size = DOUBLE)
                
    # ###########DRAW THE TITLE BAR OF A MODULE # ###########
    ...
    def moduleAttributes(self,canvas,x,y,size = "double"):
        ...
        else: raise TypeError("size: 'double'")
        ...
        if size == DOUBLE:
            ...
            canvas.roundRect(local_x,local_y,inner_width,inner_height,i_rad,1,1)
            ...
            for vital in vitals:
                ...
                canvas.setFillColorRGB(0,0,0)
                canvas.roundRect(local_x + box_dims/2, local_y, box_dims, local_height ,o_rad,1,1)
                lower_box_height = 2*padding + 2*small_box_dims + tag_height
                canvas.roundRect(local_x, local_y, vit_width, lower_box_height ,o_rad,1,1)

                canvas.setFillColorRGB(1,1,1)

                for i in range(10):
                    local_y = y - (height-local_height) - padding - (i+1)*box_dims
                    canvas.roundRect(local_x+box_dims/2,local_y,box_dims,box_dims,i_rad,1,1)
                local_y -= tag_height
                canvas.setFillColorRGB(1,1,1)
                canvas.roundRect(local_x,local_y,vit_width,tag_height,i_rad,1,1)
                canvas.setFillColorRGB(0,0,0)
                ...
                if value != 0:
                    ...
                    text += ":    "
                canvas.drawCentredString(local_x + vit_width/2, local_y+2,text)
                
                local_y -= small_box_dims
                canvas.setFillColorRGB(1,1,1)
                for col in range(5):
                    for row in range(2):
                        canvas.roundRect(local_x+col*small_box_dims,local_y-row*small_box_dims,small_box_dims,small_box_dims,1,1,1)
                vit_count -= 1
            ...
            local_y = y-height
            canvas.setFillColorRGB(0,0,0)
            canvas.roundRect(local_x,local_y,bounding_width,bounding_height,o_rad,stroke,1)
            ...
            for attrib in attribs:
                ...
                canvas.setFillColorRGB(1,1,1)
                canvas.roundRect(local_x,local_y,att_col,line_height,i_rad,stroke,1)
                canvas.setFont(FONT_NAME,text_size)
                canvas.setFillColorRGB(0,0,0)
                canvas.drawCentredString(local_x + att_col / 2, local_y + 3,attrib.upper())
                
                attr_value = self.char.getAttributeValue(attrib)
                for value in range(10):
                    canvas.setFillColorRGB(1,1,1)
                    canvas.roundRect(local_x+att_col+value*val_col,local_y,val_col,line_height,i_rad,stroke,1)
                    if attr_value > value:
                        canvas.setFillColorRGB(0,0,0)
                        canvas.roundRect(local_x+att_col+value*val_col + 1,local_y + 1,val_col - 2,line_height - 2,1,0,1)
                i -= 1

        # draw character_data ...
        data_list_1 = ["name","species","origin","concept","player"]
        data_list_2 = ["height","weight","age","gender"]
        data_list_3 = ["hair","eyes","skin","skintype"]

        data_names = {}
        data_names["name"] = msg.NAME
        data_names["species"] = msg.SPECIES
        data_names["origin"] = msg.ORIGIN
        data_names["concept"] = msg.CONCEPT
        data_names["player"] = msg.PLAYER
        data_names["height"] = msg.HEIGHT
        data_names["weight"] = msg.WEIGHT
        data_names["age"] = msg.AGE
        data_names["gender"] = msg.GENDER
        data_names["hair"] = msg.HAIR
        data_names["eyes"] = msg.EYES
        data_names["skin"] = msg.SKIN_COLOR
        data_names["skintype"] = msg.SKIN_TYPE
        ...
        # draw border
        canvas.setFillColorRGB(0,0,0)
        canvas.roundRect(local_x,local_y,width,data_height,o_rad,1,1)
        ...
        for i in range(5):
            ...
            # #draw the boxes ...
            canvas.setFillColorRGB(1,1,1)
            canvas.roundRect(local_x,local_y,data_width*3,line_height,i_rad,1,1)
            canvas.setFillColorRGB(0,0,0)
            data = data_list_1[i]
            ...
            if i < 4:
                canvas.setFillColorRGB(1,1,1)
                canvas.roundRect(local_x+data_width*3,local_y,data_width,line_height,i_rad,1,1)
                canvas.roundRect(local_x+data_width*4,local_y,data_width,line_height,i_rad,1,1)
                canvas.setFillColorRGB(0,0,0)
                ...
                data_text = self.char.getData(data)
                canvas.setFontSize(info_font)
                canvas.drawString(local_x+data_width*3+offset,local_y+offset,info_text)
                canvas.setFontSize(line_height-info_font-3)
                canvas.drawCentredString(local_x+data_width*3.5,local_y+offset+info_font+1,data_text)

                data = data_list_3[i]
                info_text = data_names[data]
                data_text = self.char.getData(data)
                canvas.setFontSize(info_font)
                canvas.drawString(local_x+data_width*4+offset,local_y+offset,info_text)
                canvas.setFontSize(line_height-info_font-3)
                canvas.drawCentredString(local_x+data_width*4.5,local_y+offset+info_font+1,data_text)
            # #xp box
            else:
                canvas.setFillColorRGB(1,1,1)
                canvas.roundRect(local_x+data_width*3,local_y,data_width*2,line_height,i_rad,1,1)
                canvas.setFillColorRGB(0,0,0)
                canvas.setFontSize(info_font)
                canvas.drawString(local_x+data_width*3+offset,local_y+offset,"XP")
                xp_avail = self.char.getAvailableXP()
                xp_total = self.char.getTotalXP()
                canvas.setFontSize(line_height-3)
                xp_text = ""
                if xp_avail != xp_total: xp_text = str(xp_avail) + " / " + str(xp_total)
                canvas.drawCentredString(local_x+data_width*4,local_y+offset+1,xp_text)
        ...
        canvas.setFillColorRGB(0,0,0)
        canvas.roundRect(local_x,local_y,bounding_width,bounding_height,o_rad,1,1)
        local_y += padding
        canvas.setFillColorRGB(1,1,1)
        img_width = bounding_width-bar
        img_height = bounding_height-2*padding
        canvas.roundRect(local_x+bar,local_y,img_width,img_height,i_rad,1,1)
        ...
        if image is not None: 
            ...
            canvas.drawImage(img,local_x+bar, local_y, width = img_width, height = img_height ,mask = "auto")
    ...
    # ##############END OF ATTRIBUTE BOX # ##############

    # ################DRAW A GENERIC BOX # ################

    def moduleNotes(self,canvas,x,y,size = SINGLE, lines_per_entry = 1, title = "NOTIZEN", info_title = None):
        ...
        if size == SINGLE: 
            ...
            height = FULL_HEIGHT
            wide = True
        else: raise TypeError("size: 'single','double',triple','full','wide','quart','big','half'")
        ...
        if info_title == None: info_line = 0
        inner_height = height - 2*padding - info_line
        line_count = int(inner_height / MINLINE_HEIGHT)
        if lines_per_entry == 0 or lines_per_entry > line_count: lines_per_entry = line_count
        while line_count%lines_per_entry != 0: line_count -= 1
        ...
        if info_title: 
            ...
            canvas.roundRect(local_x,local_y,width-bar,info_line,i_rad,stroke,1)
            offset = 2
            canvas.setFillColorRGB(0,0,0)
        ...
        while i >= 0:
            ...
            # boxes
            canvas.setFillColorRGB(1,1,1)
            canvas.roundRect(local_x,local_y,width-bar,line_height*lines_per_entry,i_rad,stroke,1)
    ...
    def moduleImage(self,canvas,x,y,size = SINGLE):
        ...
        # draw the inner box (so there is something white if the image has transparency or there is no image
        canvas.setFillColorRGB(1,1,1)
        local_height = height - 2 * padding
        local_x = x
        local_y = y - padding
        canvas.roundRect(local_x,local_y-local_height,width,local_height,i_rad,stroke,1)
        ...
        if image is not None: 
            img = ImageReader(image)
            canvas.drawImage(img,local_x , local_y - local_height, width = width - 1, height = local_height ,mask = "auto")

            # brute forcing a nice border ... 
            canvas.setFillAlpha(0)
            canvas.roundRect(local_x,local_y-local_height-padding,width,local_height+(2*padding),o_rad,stroke,1)
            canvas.roundRect(local_x,local_y-local_height-(0.5*padding),width,local_height+padding,(0.5*(o_rad+i_rad)),stroke,1)
            canvas.roundRect(local_x,local_y-local_height,width,local_height,i_rad,stroke,1)
    ...
    def moduleTraits(self,canvas,x,y,size = SINGLE, trait_type = "all", info_lines = 1, start_index = 0):
        ...
        # handle trait_type
        if trait_type == "all": title = msg.PDF_TRAITS.upper()
        elif trait_type == "positive": title = msg.PDF_POSITIVE_TRAITS.upper()
        elif trait_type == "negative": title = msg.PDF_NEGATIVE_TRAITS.upper()
        else: raise TypeError("Allowed trait_types: 'all','positive','negative'")
        ...
        canvas.roundRect(info_x,info_y,desc_width,info_line,i_rad,stroke,1)
        canvas.roundRect(info_x + desc_width,info_y,xp_width,info_line,i_rad,stroke,1)

        canvas.setFillColorRGB(0,0,0)
        canvas.setFont(FONT_NAME,7)

        offset = 2
        canvas.drawString(info_x + 4,info_y+offset,msg.PDF_NAME_AND_INFO)
        canvas.drawCentredString(info_x + desc_width + xp_width/2, info_y+offset,msg.XP)

        # filter traits ... based on trait_type
        traits = self.char.getTraits()
        trait_list = []
        if trait_type == "all": trait_list = traits
        else:
            for trait in traits:
                xp = int(trait.get("xp","0"))
                if xp >= 0 and trait_type == "positive": trait_list.append(trait) 
                elif xp < 0 and trait_type == "negative": trait_list.append(trait)
        ...
        while i >= 0:
            ...
            canvas.setFillColorRGB(1,1,1)
            # name_box
            canvas.roundRect(local_x,local_y,desc_width,line_height,i_rad,stroke,1)
            # xp_box
            local_x += desc_width
            canvas.roundRect(local_x,local_y,xp_width,line_height,i_rad,stroke,1)
            # info_box
            if info_lines > 0: 
                box_height = info_lines * line_height
                temp_y = local_y - box_height
                local_x = x + bar
                canvas.roundRect(local_x,temp_y,width-bar,box_height,i_rad,stroke,1)
            ...
            if trait_count < len(trait_list):
                ...
                if (full_trait is not None) and (selected is not None): 
                    ...
                    if full_ranks:
                        ...
                        if (rank): trait_rank = "["+rank+"]"

                    # description (base)
                    description = []
                    description.append(trait.find("description"))
                    description.append(full_trait.find("description"))
                    ...
                    for variable in full_variables:
                        ...
                        if var_name:
                            ...
                            description.append(variable.find("description[@value='"+value+"']"))
                
                canvas.setFillColorRGB(0,0,0)
                local_x = x + bar

                for desc in description:
                    if desc == None: description.remove(desc)
                ...
                if info_lines == 0:
                    ...
                    while v_count < len(trait_vars):
                        if v_count == 0: trait_name += "("
                        trait_name += trait_vars[v_count][1]
                        v_count += 1
                        if v_count < len(trait_vars): trait_name += ","
                        else: trait_name += ")"

                # and trim ...                
                if len(trait_name) > limit: trait_name = trait_name[0:limit] + "..."
                canvas.drawString(local_x + offset, local_y + offset,trait_name)

                # xp
                canvas.drawCentredString(local_x + desc_width + xp_width/2,local_y + offset,trait_xp)
                ...
                if info_lines > 0: 
                    ...
                    while v_count < len(trait_vars) and cur_line < info_lines:
                        var_text = trait_vars[v_count][0] + ": " + trait_vars[v_count][1]
                        ...
                        v_count += 1
                        if v_count<len(trait_vars): text += ", " 
                    ...
                    while cur_line < info_lines:
                        if desc_count < len(description):
                            ...
                            while word_count < len(desc_words):
                                if len(text) + len(desc_words[word_count]) < info_limit:
                                    text += " "+desc_words[word_count]
                                    word_count += 1
                                else: 
                                    local_y -= line_height
                                    cur_line += 1
                                    if cur_line == info_lines and word_count < len(desc_words): text += "..."
                                    canvas.drawString(local_x + offset, local_y + offset, text)
                                    text = ""
                                    if cur_line == info_lines: break
                            # the final line ... 
                            if text != "":
                                local_y -= line_height
                                cur_line += 1
    ...
    def moduleSkills(self,canvas,x,y,size = SINGLE, skill_type = "all",start_index = 0):
        ...
        if skill_type == "all": title = msg.PDF_SKILLS.upper()
        elif skill_type == "active": title = msg.PDF_SKILLS_ACTIVE.upper()
        elif skill_type == "passive": title = msg.PDF_SKILLS_PASSIVE.upper()
        elif skill_type == "knowledge": title = msg.PDF_SKILLS_KNOWLEDGE.upper()
        elif skill_type == "lang": title = msg.PDF_SKILLS_LANGUAGE.upper()
        else: raise TypeError("skill_type: string - 'all','active','passive','knowledge','lang'")
        ...
        info_y = y - padding - info_line
        info_x = x + bar
        canvas.roundRect(info_x,                         info_y,spec_col,info_line,i_rad,stroke,1)
        canvas.roundRect(info_x + spec_col,              info_y,skill_col,info_line,i_rad,stroke,1)
        canvas.roundRect(info_x + spec_col + 1*skill_col,info_y,skill_col,info_line,i_rad,stroke,1)
        canvas.roundRect(info_x + spec_col + 2*skill_col,info_y,skill_col,info_line,i_rad,stroke,1)
        canvas.roundRect(info_x + spec_col + 3*skill_col,info_y,desc_col,info_line,i_rad,stroke,1)

        canvas.setFillColorRGB(0,0,0)
        canvas.setFont(FONT_NAME,7)

        offset = 2
        canvas.drawCentredString(info_x + 0.5*spec_col            ,info_y+offset,"S")
        canvas.drawCentredString(info_x + spec_col + 0.5*skill_col,info_y+offset,"1")
        canvas.drawCentredString(info_x + spec_col + 1.5*skill_col,info_y+offset,"2")
        canvas.drawCentredString(info_x + spec_col + 2.5*skill_col,info_y+offset,"3")
        canvas.drawString(info_x + spec_col + 3*skill_col + 3,info_y+offset,msg.PDF_SKILLNAME)

        # filter skills ... based on skill_type
        skills = self.char.getSkills()
        skill_list = []
        if skill_type == "all": skill_list = skills
        else:
            if skill_type == "knowledge": skill_type = "passive"
            elif skill_type == "passive": skill_type = ["passive","lang"]
        ...
        while i >= 0:
            ...
            if skill_count < len(skill_list):
                ...
                if len(skill_name) > limit: skill_name = skill_name[0:limit] + "..."
                skill_spec_val = skill.get("spec","1")
                if skill_spec_val == "1": skill_spec = "G"
                if skill_spec_val == "2": skill_spec = "F"
                if skill_spec_val == "3": skill_spec = "S"

            # construct the line ... 
            canvas.setFillColorRGB(1,1,1)
            # #spec box
            canvas.roundRect(line_x, line_y,spec_col,line_height,i_rad,stroke,1)
            # #skill 1
            line_x += spec_col
            canvas.roundRect(line_x,line_y,skill_col,line_height,i_rad,stroke,1)
            if skill_value >= 1: 
                canvas.setFillColorRGB(0,0,0)
                canvas.roundRect(line_x + 1,line_y + 1,skill_col - 2,line_height - 2,1,0,1)
            # skill 2
            line_x += skill_col
            canvas.setFillColorRGB(1,1,1)
            canvas.roundRect(line_x,line_y,skill_col,line_height,i_rad,stroke,1)
            ...
            line_x += skill_col
            canvas.setFillColorRGB(1,1,1)
            canvas.roundRect(line_x,line_y,desc_col,line_height,i_rad,stroke,1)
            canvas.setFillColorRGB(0,0,0)
            canvas.drawString(x+bar+4,line_y+3,skill_spec)
            canvas.drawString(line_x+4,line_y+3,skill_name)
    ...
    # ##############DRAW A EQUIPMENT BOX # ##############

    def moduleEquipment(self,canvas,x,y,size = SINGLE, item_id = -1, condensed = False,equipped = False, content = False, 
                        item_type = None, exclude = None, content_exclude = None, info_lines = 0):
        ...
        if item_id >= 0:
            ...
            if item is not None: 
                ...
                if container is not None: title = container.get("name").upper()
        # based on item_type
        elif item_type is not None:
            if it.CLOTHING in item_type: title = msg.PDF_CLOTHING_AND_MORE.upper()
            melee_weapons = [it.CLUBS,it.NATURAL,it.STAFFS,it.BLADES]
            if any(melee_weapons,item_type) and len(item_type) > 1: title = msg.PDF_MELEE.upper()
            guns = [it.PISTOLS,it.REVOLVERS,it.RIFLES,it.RIFLES_SA,
                    it.SHOT_GUNS,it.SHOT_GUNS_SA,it.AUTOMATIC_PISTOLS,
                    it.MASCHINE_GUNS,it.AUTOMATIC_RIFLES,it.BLASTER]
            if any(guns,item_type) and len(item_type) > 1: 
                if title == msg.PDF_MELEE: title = msg.PDF_WEAPONS.upper()
                else: title = msg.PDF_GUNS.upper()
        ...
        if item_id >= 0: 
            ...
            if item is not None: 
                ...
                for id in item_ids:
                    ...
                    if sub_item is not None: item_list.append(sub_item)
        else:
            # get the full list ... 
            ...
            if condensed:
                for item in items:
                    ...
                    valid_item = False
                    # check item_type ... 
                    if item_type is None: valid_item = True 
                    elif i_type in item_type: valid_item = True
                    # check exclude
                    if valid_item and i_type in exclude: valid_item = False
                            ...
                            if some_name == name:
                                ...
                                for option in options: 
                                    ...
                                    for some_option in some_options:
                                        ...
                                        if option_name == s_option_name and option_value != s_option_value:
                                            identical = False
                                            break
                                ...
                                if identical: 
                                    ...
                                    some_item.set("quantity",str(quantity+some_quantity))
                                    item_added = True
                        # ... or add the item to the list ... 
                        if item_added == False: item_list.append(item)
                ...
                if equipped == False: 
                    for item in items: 
                        ...
                        if item_type is None and item.get("type") not in exclude: item_list.append(item)
                        if item_type is not None: 
                            if item.get("type") in item_type: item_list.append(item)
                
                # equipped items, show no contents ... 
                elif equipped == True and content == False: 
                    for item in items: 
                ...
                elif equipped == True and content == True: 
                    for item in items: 
                        if item.get("equipped","0") == "1":
                            # check for item_list and exclude ... 
                            item_included = False
                            ...
                            # and the type not listed in the exclude_content list ...                             
                            
                            if item_included and item.get("type") not in content_exclude:
                                item_ids = item.get("content","")
                                item_ids = item_ids.split()
                                for id in item_ids:
                                    sub_item = self.char.getItemById(str(id))
        ...
        canvas.roundRect(info_x,info_y,quality_width,info_line,i_rad,stroke,1)
        info_x += quality_width
        canvas.roundRect(info_x,info_y,amount_width,info_line,i_rad,stroke,1)
        info_x += amount_width
        canvas.roundRect(info_x,info_y,text_width,info_line,i_rad,stroke,1)
        info_x += text_width
        canvas.roundRect(info_x,info_y,weight_width,info_line,i_rad,stroke,1)
        ...
        info_x = x + bar
        offset = 2
        canvas.drawCentredString(info_x + quality_width/2 ,info_y+offset,"Q")
        info_x += quality_width
        canvas.drawCentredString(info_x + amount_width/2 ,info_y+offset, msg.PDF_QUANTITY)
        info_x += amount_width
        canvas.drawString(info_x + offset,info_y+offset,msg.PDF_ITEMNAME)
        info_x += text_width
        canvas.drawCentredString(info_x + weight_width / 2, info_y+offset,msg.PDF_WEIGHT)
        ...
        while i >= 0:
            ...
            local_x = x + bar
            canvas.setFillColorRGB(1,1,1)
            canvas.roundRect(local_x,local_y,quality_width,line_height-spacer,i_rad,stroke,1)
            local_x += quality_width
            canvas.roundRect(local_x,local_y,amount_width,line_height-spacer,i_rad,stroke,1)
            local_x += amount_width
            canvas.roundRect(local_x,local_y,text_width,line_height-spacer,i_rad,stroke,1)
            local_x += text_width
            canvas.roundRect(local_x,local_y,weight_width,line_height-spacer,i_rad,stroke,1)
            local_x = x + bar
            if info_lines > 0: 
                box_height = info_lines * line_height
                temp_y = local_y - box_height
                if i < 0: spacer = 0
                canvas.roundRect(local_x,temp_y+spacer,width-bar,box_height-spacer,i_rad,stroke,1)

            # add item to info-box ...
            if item_selector < len(item_list):
                item = item_list[item_selector]
                item_name = item.get("name","")
                item_quality = item.get("quality","7")
                ...
                canvas.setFillColorRGB(0,0,0)
                local_x = x + bar
                canvas.drawCentredString(local_x + quality_width/2.0, local_y + 3,item_quality)
                local_x += quality_width
                canvas.drawRightString(local_x + amount_width - 3, local_y + 3,item_quantity)
                local_x += amount_width
                canvas.drawString(local_x + 3, local_y + 3,item_name)
                local_x += text_width
                canvas.drawRightString(local_x + weight_width - 3, local_y + 3,item_weight)
                ...
                if info_lines > 0: 
                    ...
                    text = text.strip()
                    if len(text) > limit: text = text_2
                    
                    # print text ... 
                    if len(text) > 0: 
                        cur_line += 1
                        local_y -= line_height
                        canvas.drawString(local_x + 3, local_y + 3,text)
                    ...
                    word_count = 0
                    text = ""
                    while word_count < len(desc_words) and cur_line <= info_lines:
                        while len(text) + len(desc_words[word_count]) <= limit: 
                            text += desc_words[word_count] + " "
                            word_count += 1
                            if word_count == len(desc_words): break
                        local_y -= line_height
                        cur_line += 1
                        if cur_line == info_lines and word_count < len(desc_words): text += " ..."
                        canvas.drawString(local_x + 3, local_y + 3,text)
    ...
    # ##############END OF EQUIPMENT BOX # ###############

    # ##############DRAW A WEAPONS BOX # ##############

    def moduleWeapons(self,canvas,x,y,size = SINGLE, 
                      equipped = False, variant = None, item_type = None, 
                      amount = False, info_lines = 0):
        """ This module is used to render a wide range of inventory related elements on the character sheet
            for this reason please refer to the notes on which variables to combine ... 
        canvas: reportlab pdf canvas
        x,y: int - top left corner in points
        ...
        if variant is not None:
            if variant == "melee": 
                ...
                item_type = [it.CLUBS,it.NATURAL,it.STAFFS,it.BLADES,it.OTHER_MELEE,it.TOOLS]
            elif variant == "guns": 
                title = msg.PDF_GUNS.upper()
                item_type = [it.PISTOLS,it.REVOLVERS,it.RIFLES,it.RIFLES_SA,
                             it.SHOT_GUNS,it.SHOT_GUNS_SA,it.AUTOMATIC_PISTOLS,
                             it.MASCHINE_GUNS,it.AUTOMATIC_RIFLES,it.BLASTER]
            elif variant == "ammo": 
                title = msg.PDF_AMMO
            ...
            for item in items: 
                ...
                if item_type is None: 
                    if item.find("damage") is not None:
                        if item.get("type") != it.AMMO: item_list.append(item)
                else: 
                    if item.get("type") in item_type: 
                        if item.find("damage") is not None: item_list.append(item)
        else:                
            for item in items: 
                if item.get("equipped","0") == "1": 
                    # check for item_list and exclude ... 
            ...
            for item in item_list:
                ...
                for new_item in new_list:
                    ...
                    if new_name == name:
                        ...
                        new_cal = "0"
                        new_col = "0"
                        new_cal_tag = item.find("option[@name='"+it.OPTION_CALIBER+"']")
                        if new_cal_tag is not None:
                            new_cal = cal_tag.get("value","0")
                        new_col_tag = item.find("option[@name='"+it.OPTION_CALIBER+"']")
                        ...
                        if new_cal == cal and new_col == col:
                            ...
                            item_added = True
                # ... or add the item to the list ... 
                if item_added == False: new_list.append(item)
        ...
        info_x = x + bar
        quantity_width = 0
        if amount: quantity_width = 1.5 * line_height
        damage_width = 2.0*line_height
        name_width = width - bar - damage_width - quantity_width

        # limit characters per line... 
        info_limit = limit + 8
        if quantity_width > 0: limit -= 8
        
        if amount: 
            canvas.roundRect(info_x,info_y,quantity_width,info_line,i_rad,stroke,1)
            info_x += quantity_width
        canvas.roundRect(info_x,info_y,name_width,info_line,i_rad,stroke,1)
        info_x += name_width
        canvas.roundRect(info_x,info_y,damage_width,info_line,i_rad,stroke,1)
        canvas.setFillColorRGB(0,0,0)
        canvas.setFont(FONT_NAME,7)
        info_x = x + bar
        offset = 2
        if amount:
            canvas.drawCentredString(info_x + quantity_width/2 ,info_y+offset,msg.PDF_QUANTITY)
            info_x += quantity_width
        canvas.drawString(info_x + offset ,info_y+offset,msg.PDF_ITEMNAME)
        info_x += name_width
        canvas.drawCentredString(info_x + damage_width/2 ,info_y+offset,msg.PDF_DAMAGE)
        ...
        while i >= 0:
            ...
            canvas.setFillColorRGB(1,1,1)
            if amount:
                canvas.roundRect(local_x,local_y,quantity_width,line_height-spacer,i_rad,stroke,1)
                local_x += quantity_width
            canvas.roundRect(local_x,local_y,name_width,line_height-spacer,i_rad,stroke,1)
            local_x += name_width
            canvas.roundRect(local_x,local_y,damage_width,line_height-spacer,i_rad,stroke,1)
            ...
            if item_selector < len(item_list):
                ...
                if caliber_tag is not None: caliber = caliber_tag.get("value")
                ...
                if amount:
                    ...
                    canvas.drawRightString(local_x + quantity_width - offset, local_y + offset,item_quantity)
                    local_x += quantity_width

                if lines_per_entry == 1 and caliber: item_name += " ["+caliber+"]"
                if len(item_name) > limit: item_name = item_name[0:limit]+"..."
                canvas.drawString(local_x + offset, local_y + offset,item_name)
                
                local_x += name_width
                canvas.drawCentredString(local_x + damage_width/2, local_y + offset,item_damage)
                ...
                if info_lines > 0:
                    ...
                    if ammo_tag is not None: 
                        ...
                        if number_chambers > 1: 
                            if info_lines == 1: capacity_text = str(number_chambers)                            
                            else: capacity_text = str(number_chambers)+ " " + msg.PDF_CHAMBERS
                        else: 
                                ...
                                if sub_item is not None:
                                    ...
                                    if sub_type == "Clip":
                                        ...
                                        if sub_container is not None:
                                            if info_lines == 1: capacity_text = sub_container.get("size","0")
                                            else: capacity_text = sub_container.get("size","0") + msg.PDF_CLIPSIZE
                    # caliber ... 
                    caliber_text = ""
                    if caliber is not None:
                        if info_lines == 1: caliber_text = caliber
                        else: caliber_text = msg.PDF_CALIBER+ ": " +caliber
