...
msg = config.Messages()
#this class creates a trait selection window        
class TraitSelector:
    def __init__(self,main):
        
        # point to character and traitstree
        self.char = main.char
        self.all_traits = main.traits
        self.main = main
        ...
        # self.trait_editor.bind("<FocusOut>", lambda x: self.trait_editor.destroy())
        ...
        self.search_mode_name = tk.Button(self.upper_search_frame, text = msg.TS_NAME, relief = tk.SUNKEN, command =  lambda: self.switchSearchMode("name"))
        self.search_mode_name.pack(side = tk.LEFT, fill = tk.X, expand = 1)
        self.search_mode_cls = tk.Button(self.upper_search_frame, text = msg.TS_CLASS, command =  lambda: self.switchSearchMode("cls"))
        self.search_mode_cls.pack(side = tk.LEFT, fill = tk.X, expand = 1)
        self.search_mode_grp = tk.Button(self.upper_search_frame, text = msg.TS_GROUP, command =  lambda: self.switchSearchMode("grp"))
        ...
        self.search_box = tk.Entry(self.lower_search_frame,textvariable = self.search)
        ...
        self.search_button = tk.Button(self.lower_search_frame,text = msg.TS_SEARCH, command = self.searchTraits)
        ...
        self.scrollbar_select = tk.Scrollbar(self.select_frame,orient = tk.VERTICAL)
        self.list_box = tk.Listbox(self.select_frame)
        self.list_box.config(yscrollcommand = self.scrollbar_select.set, height = 20, width = 30)
        ...
        self.info_xp = tk.Label(self.info_title_frame, textvariable = self.trait_cost, font = "Arial 12 bold")
        self.info_xp.pack(side = tk.RIGHT)
        self.info_title = tk.Label(self.info_title_frame, text = msg.TS_TRAIT, font = "Arial 12 bold", justify = tk.LEFT)
        ...
        self.info_scrollbar = tk.Scrollbar(self.info_description_frame,orient = tk.VERTICAL)
        ...
        # the submit button
        self.submit = tk.Button(self.info_frame, text = msg.TS_ADD_TRAIT, command = self.addTrait)
    ...
    def displayTraitInfo(self,event):
        ...
        if specification is not None:
            ...
            specification_label = tk.Label(specification_frame, text = specification_label_text+": ")
            specification_label.pack(side = tk.LEFT)
            # create a stringvar
            self.info_subtrait_vars["spec"] = tk.StringVar()
            self.info_subtrait_vars["spec"].set("")
            self.info_subtrait_vars["spec"].trace("w", self.variableChanged)
            # bind the tcl_name of the variable to the variable
            self.info_subtrait_tcl[str(self.info_subtrait_vars["spec"])] = "spec"
            self.info_subtrait_widgets["spec"] = specification_entry = tk.Entry(specification_frame, textvariable = self.info_subtrait_vars["spec"])
        ...
        if (ranks):
            for rank in ranks:
                ...
                min = int(rank.get("min","1"))
                max = int(rank.get("max","10"))
                rank_frame = tk.Frame(self.info_subtrait)
                rank_label = tk.Label(rank_frame, text = rank_name)
                rank_label.pack(side = tk.LEFT)
                self.info_subtrait_vars["rank"+"_"+rank_id] = var = tk.StringVar()
                var.set(min)
                var.trace("w", self.variableChanged)
                # bind the label to the tcl_name
                self.info_subtrait_tcl[str(self.info_subtrait_vars["rank_"+rank_id])] = "rank_"+rank_id
                self.info_subtrait_widgets["rank"+rank_id] = spinbox = tk.Spinbox(rank_frame, textvariable = var, from_ = min, to = max)
        ...
        if (variables):
            ...
            for variable in variables:
                ...
                variable_label = tk.Label(variable_frame, text = variable_label_text)
                variable_label.grid(row = rownum, column = 0)
                # create a string_var for the variable
                self.info_subtrait_vars["var"+"_"+variable_id] = var = tk.StringVar()
                var.trace("w", self.variableChanged)
                # bind the label to the tcl_name
                self.info_subtrait_tcl[str(self.info_subtrait_vars["var"+"_"+variable_id])] = "var"+"_"+variable_id
                self.info_subtrait_widgets["var"+"_"+variable_id] = spinbox = tk.Spinbox(variable_frame, textvariable = var, values = variable_values)
    ...
    def xpCostChanged(self, tcl_var, empty, type):
    ...
    def variableChanged(self, tcl_name, empty, type):
        trait_cost = 0
        ...
        if len(ranks) > 0:
            for rank in ranks:
                ...
                try:
                    selected = self.info_subtrait_vars["rank_"+id].get()
                    operand = rank.get("operand")
                    if (operand is None): operand = "add"
                    try: 
                    ...
                    if (operand == "add"):
                        trait_cost = trait_cost + (selected * xp_factor)
        ...
        if len(variables) > 0:        
            for variable in variables:
                ...
                # we have to try because the variables have not yet all been created
                try:
        ...
        if (trait_cost < 1 and trait_cost > 0): trait_cost = 1
        if (trait_cost > -1 and trait_cost < 0): trait_cost = -1
    ...
    def switchSearchMode(self,mode):
        if (mode != self.search_mode):
            ...
            if (mode == "cls"):
                ...
                self.search_box.pack_forget()
                self.search_box = tk.Spinbox(self.lower_search_frame,textvariable = self.search)
                self.search_box.pack(fill = tk.BOTH)
                self.search_box.bind("<Return>",lambda event: self.searchTraits())
                ...
                trait_set = set()
                for trait in traits:
                    if trait[1]: trait_set.add(trait[1])
            ...
            if (mode == "grp"):
                ...
                trait_set = set()
                for trait in traits:
                    if trait[2]: trait_set.add(trait[2])
                trait_list = list(trait_set)
                self.search_box.config(values = trait_list)
                self.search.set(msg.TS_JUST_SCROLL)
    ...
    def addTrait(self):
        self.char.addTrait(self.trait,self.info_subtrait_vars,self.trait_cost,self.trait_description)

        # finally close the window
        self.main.updateTraitList()
        self.close()

    def searchSelectAll(self,event):
        widget = event.widget
    ...
    def close(self):
        ...
        self.main.open_windows["trait"] = 0
