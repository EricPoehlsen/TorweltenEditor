...
class LayoutScreen(tk.Frame):
    ...
    def __init__(self,main,app):
        ...
        self.mod_types = {page.MOD_ATTRIBUTES:msg.PDF_ATTRIBUTES,
        ...
        self.cur_page.set(str(self.active_page) + " / " + str(self.pages))
    ...
    def showToolbar(self,frame):
        new_template = tk.Button(frame,text = msg.SL_NEW,command = self._newTemplate)
        new_template.pack(side = tk.LEFT)

        load_template = tk.Button(frame,text = msg.SL_LOAD, command = self._loadTemplate)
        load_template.pack(side = tk.LEFT)

        save_template = tk.Button(frame,text = msg.SL_SAVE, command = self._saveTemplate)
    ...
    def showPage(self,frame):
        ...
        for row in range(4):    
            for col in range(4):
                ...
                text = msg.SL_EMPTY
                text_col = "#dddddd"
                module = page.find("module[@id='"+id+"']")
                self.widgets[position] = slot = tk.Canvas(frame, width = 150, height = 100, borderwidth = 0, highlightthickness=0, relief=tk.RIDGE,bg = "#ffffff")
                ...
                if module is not None:
                    ...
                    # draw box ,,, 
                    slot.create_polygon(x1,y1+2*r, x1,y1+2*r, x1,y1+r, x1+r,y1, x1+2*r,y1, x1+2*r,y1,
                                        x2-2*r,y1, x2-2*r,y1, x2-r,y1, x2,y1+r, x2,y1+2*r, x2,y1+2*r,
                                        x2,y2-2*r, x2,y2-2*r, x2,y2-r, x2-r,y2, x2-2*r,y2, x2-2*r,y2,
                                        x1+2*r,y2, x1+2*r,y2, x1+r,y2, x1,y2-r, x1,y2-2*r, x1,y2-2*r,
                                        smooth = 1,fill = "#ffffff",outline="#000000",width = 2)
                ...
                slot.bind("<Button-1>",lambda event, position = position: self._editModule(position)) 
                

    def showControl(self,frame):
        sub_frame = tk.Frame(frame)
        icon = ImageTk.PhotoImage(file = "ui_img/book_previous.png")
        self.widgets["last_page"] = tk.Button(sub_frame,image = icon, command = lambda:self._switchPage(-1))
        self.widgets["last_page"].image = icon
        self.widgets["last_page"].pack(side=tk.LEFT,fill = tk.X)
        
        self.widgets["cur_page"] = tk.Label(sub_frame,textvariable = self.cur_page)
        self.widgets["cur_page"].pack(side=tk.LEFT,fill = tk.X)

        icon = ImageTk.PhotoImage(file = "ui_img/book_next.png")
        self.widgets["next_page"] = tk.Button(sub_frame,image = icon, command = lambda:self._switchPage(+1))
        ...
        self.widgets["new_page"] = tk.Button(sub_frame,image = icon, command = self._newPage)
        self.widgets["new_page"].image = icon
        self.widgets["new_page"].pack(side=tk.LEFT)

        icon = ImageTk.PhotoImage(file = "ui_img/page_up.png")
        self.widgets["page_up"] = tk.Button(sub_frame,image = icon,command = lambda:self._movePage(self.active_page-1))
        self.widgets["page_up"].image = icon
        self.widgets["page_up"].pack(side = tk.LEFT)

        icon = ImageTk.PhotoImage(file = "ui_img/page_down.png")
        self.widgets["page_down"] = tk.Button(sub_frame,image = icon,command = lambda:self._movePage(self.active_page+1))
        ...
        self.widgets["del_page"] = tk.Button(sub_frame,image = icon, command = self._deletePage)
        self.widgets["del_page"].image = icon
        self.widgets["del_page"].pack(side=tk.LEFT)

        sub_frame.pack(fill = tk.X, expand = 1)

        export = tk.Button(frame,text = msg.SL_EXPORT,command = self._exportPDF)
    ...
    def _movePage(self,num):
        if 1 <= num <= self.pages:
            other_page = self.template.find("page[@num='"+str(num)+"']")
            active_page = self.template.find("page[@num='"+str(self.active_page)+"']")

            other_page.set("num",str(self.active_page))
            active_page.set("num",str(num))

            self._refactorTemplate()
            self._switchPage(num,True)
    ...
    def _deletePage(self):
        ...
        if self.active_page < self.pages:
            for page_num in range (self.active_page+1,self.pages+1):
                page = self.template.find("page[@num='"+str(page_num)+"']")
                page.set("num",str(page_num-1))
    ...
    def _refactorTemplate(self):
        ...
        for page in pages:
            ...
            for module in modules:
                module_sorter.append((module.get("col")+module.get("row"),module))
    ...
    def _loadTemplate(self):
        ...
        options['filetypes'] = [('Template', '.xml')]
        options['initialdir'] = './templates'
        options['initialfile'] = 'template.xml'
        options['parent'] = self
        options['title'] = 'Charakterbogentemplate laden ...'
    ...
    def _saveTemplate(self):
        ...
        options['initialfile'] = "template.xml"
        options['parent'] = self
        options['title'] = 'Charakterbogentemplate speichern ...'
    ...
    def _generateGrid(self):
        ...
        for mypage in pages:
            ...
            for module in modules: 
                ...
                rows = 1
                cols = 1
                if size == page.DOUBLE: rows = 2
                if size == page.TRIPLE: rows = 3
                if size == page.FULL: rows = 4
                if size == page.WIDE:
                    rows = 1
                    cols = 2
                if size == page.QUART: 
