...
class EquipmentScreen(tk.Frame):
    def __init__(self,main,app):
        ...
        self.left_frame.pack(side = tk.LEFT, anchor = tk.N, fill = tk.Y)
        # this frame handles the initial account setting of the character
        initial_account_frame = tk.Frame(self.left_frame)
        reduce_account = tk.Button(initial_account_frame,text = "-",command = lambda:self.initialAccount(-1))
        reduce_account.pack(side = tk.LEFT, anchor = tk.W)
        account_label = tk.Label(initial_account_frame, textvariable = self.account_info)
        account_label.pack(side = tk.LEFT, fill = tk.X, expand = 1)
        increase_account = tk.Button(initial_account_frame,text = "+",command = lambda:self.initialAccount(+1))
        increase_account.pack(side = tk.LEFT, anchor = tk.E)
        initial_account_frame.pack(fill = tk.X, expand = 1)

        # a button to buy new stuff (opens the InventoryEditor)
        self.buy_button = tk.Button(self.left_frame, text = msg.ES_BUY_BUTTON, command = self.displayInventoryEditor)
        self.buy_button.pack(fill = tk.X)

        # show equipped items 
        self.equipped_frame = tk.LabelFrame(self.left_frame, text = msg.ES_EQUIPPED, font = "Arial 10 bold")
        self.equipped_canvas = tk.Canvas(self.equipped_frame,width = 250,height = 450)
        self.equipped_canvas.pack(side = tk.LEFT)
        self.equipped_scroll = tk.Scrollbar(self.equipped_frame,orient = tk.VERTICAL)
        ...
        self.melee_frame = tk.LabelFrame(self.center_frame, text = msg.ES_MELEE, font = "Arial 10 bold")
        
        self.melee_canvas = tk.Canvas(self.melee_frame,width = 230, height = 250)
        ...
        self.guns_frame = tk.LabelFrame(self.center_frame,text = msg.ES_GUNS, font = "Arial 10 bold")
        self.guns_canvas = tk.Canvas(self.guns_frame,width = 230, height = 250)
        ...
        self.unassigned_frame = tk.LabelFrame(self.right_frame,text = msg.ES_UNASSIGNED, font = "Arial 10 bold")
        self.unassigned_canvas = tk.Canvas(self.unassigned_frame,height = 500,width = 230)
        self.unassigned_canvas.pack(side = tk.LEFT)
        self.unassigned_scroll = tk.Scrollbar(self.unassigned_frame,orient = tk.VERTICAL)
    ...
    def initialAccount(self,change = 0):
        ...
        # modify the current value - minimum is 0
        ...
        text = msg.ES_INITIAL_FUNDS %(str(initial_amount))
    ...
    def showUnassignedItems(self):
        ...
        for item in items:
            ...
            if inside > 0: unassigned = False
            equipped = int(item.get("equipped","0"))
            if equipped == 1: unassigned = False
            ...
            if unassigned: 
                ...
                amount_label = tk.Label(item_frame, text = str(quantity)+msg.MULTIPLY, justify = tk.LEFT, width = 4)
                amount_label.grid(row = 0, column = 0, sticky = tk.E)
                name_label = tk.Label(item_frame, text = item.get("name"), justify = tk.LEFT)
                name_label.bind("<Button-1>",lambda event, id=id: self.displayItemEditor(event,id)) 
                name_label.grid(row = 0, column = 1, sticky = tk.W)
                    
                if item.get("type") in equippable_types: 
                    equip_button = tk.Button(item_frame, text = "E")
                    equip_button.bind("<Button-1>",lambda event, id=id: self.equipItem(event,id))
                ...
                if self.active_bag_id >= 0: 
                    ...
                    pack_button.bind("<Button-1>",lambda event, id=id: self.packItem(event,id)) 
                    pack_button.grid(row = 0, column = 3)
                else:
                    empty = tk.Label(item_frame, text = " ", width = 2)
                    empty.grid(row = 0, column = 3)
                
                canvas.create_window(0,y,window = item_frame,anchor = tk.NW,width = 230)
    ...
    def showEquippedItems(self):
        ...
        # armor and clothing
        armor_frame = tk.LabelFrame(canvas,text = msg.ES_CLOTHING_ARMOR,width = 200)
        self.showEquippedClothing(armor_frame)
        armor = canvas.create_window(0,y,window = armor_frame,anchor = tk.NW,width = 250)
        self.update_idletasks()
        ...
        for bag in bags:
            ...
            canvas.create_window(0,y,window = bag_frame,anchor = tk.NW,width = 250)
            self.update_idletasks()
            y += bag_frame.winfo_height()
        # tk.Label(self.equipped_frame,text = " ", width = 30).pack()
    ...
    def showBagContents(self,bag,frame):
        ...
        if item_ids is not None: 
            ...
            for item_id in entries:
                ...
                if item is not None: 
                    ...
                    quantity_label = tk.Label(item_line,text = item.get("quantity")+msg.MULTIPLY)
                    quantity_label.pack(side = tk.LEFT)
                    item_label = tk.Label(item_line,text = item.get("name"))
                    item_label.bind("<Button-1>",lambda event,id = item_id:self.displayItemEditor(event,id))
                    item_label.pack(side = tk.LEFT,fill = tk.X, expand = 1)
                    item_unpack = tk.Button(item_line, text = "U")
                    item_unpack.bind("<Button-1>", lambda event, id = item_id: self.unpackItem(event, id))
        ...
        bag_unequip.bind("<Button-1>", lambda event, id = str(bag_id): self.unequipItem(event, id))
        bag_unequip.pack(fill = tk.X)        
        
        bag_frame.pack(fill = tk.BOTH, expand = 1)
        bag_frame.bind("<Button-1>",lambda event, bag_id = bag_id: self.setActiveBag(event, bag_id))
    ...
    def showEquippedClothing(self,frame):
        ...
        for item in items:
            ...
            item.get("type") == it.HARNESS)):
                ...
                if damage is not None: sd = damage.get("value","")
                ...
                id = item.get("id")
                unequip_button.bind("<Button-1>",lambda event, id = id: self.unequipItem(event,id))
    ...
    def showEquippedMelee(self,canvas):
        ...
        for item in items:
            if item.get("equipped","0") == "1":
                ...
                weapons = [it.CLUBS,it.BLADES,it.STAFFS,it.OTHER_MELEE,it.TOOLS,it.NATURAL]
                if item_type in weapons:
                    ...
                    if damage is not None: 
                        ...
                        name_label.bind("<Button-1>", lambda event, id = item_id: self.displayItemEditor(event,id))
    ...
    def showEquippedGuns(self,canvas):
        ...
        for item in items:
            if item.get("equipped","0") == "1":
                item_type = item.get("type")
                weapons = [it.PISTOLS,it.REVOLVERS,it.RIFLES,it.SHOT_GUNS,it.RIFLES_SA,
                           it.SHOT_GUNS_SA,it.AUTOMATIC_PISTOLS,it.AUTOMATIC_RIFLES,it.MASCHINE_GUNS]
                if item_type in weapons:
                    item_name = item.get("name")
                    ...
                    if ammo_tag is not None: 
                        ...
                        for chamber in ammo_list:
                            if chamber != "-1": loaded_chambers = loaded_chambers + 1
                    ...
                    if content != "x":
                        ...
                        for content_id in content_list:
                            ...
                            if content_item is not None:
                                ...
                                if content_type == it.CLIP: loaded_clip = content_item

                    # building the display ...
                    item_id = item.get("id")
                    weapon_frame = tk.Frame(canvas, borderwidth = 2, relief = tk.RIDGE)
                    name_line = tk.Frame(weapon_frame)
                    name_label = tk.Label(name_line, text = item_name, justify = tk.LEFT)
                    name_label.bind("<Button-1>", lambda event, id = item_id: self.displayItemEditor(event,id))
                    ...
                    if chambered_item is None:
                        ...
                        damage_label.pack(side = tk.RIGHT, anchor = tk.E)
                        chamber_label = tk.Label(chambered_line, text = msg.ES_NOT_LOADED)
                        chamber_label.pack()
                    else: 
                        chambered_name = chambered_item.get("name")
                        damage = chambered_item.find("damage").get("value")
                        chamber_label = tk.Label(chambered_line, text = chambered_name)
                    ...
                    if loaded_clip is None:
                        ammo_text = msg.ES_CHAMBERED_AMMO %(chambers, loaded_chambers)
                        ...
                        if clip_content[0] == "x" or clip_content[0] == "":
                            pass
                        else: ammo_in_clip = len(clip_content)
                        ammo_text = "%s - %s / %s" %(clip_name, str(ammo_in_clip), str(clip_size))
                        ammo_label = tk.Label(ammo_line,text = ammo_text)
                        ammo_label.pack(fill = tk.X, expand = 1)
                    ...
                    canvas.create_window(0,y,window = weapon_frame,anchor = tk.NW, width = 230)
                    self.update_idletasks()
                    y += weapon_frame.winfo_height()
        
    # called when item shall be equipped
    def equipItem(self,event,id):
        item = self.char.getItemById(id)
        quantity = int(item.get("quantity"))
        if quantity > 1: 
    ...
    def unequipItem(self,event,id):
        item = self.char.getItemById(id)
        if int(self.active_bag_id) == int(id):
            self.active_bag_id = -1
        self.char.unequipItem(item)
        self.updateItemList()

    # pack an item into the active bag
    def packItem(self,event,id):
        item = self.char.getItemById(str(id))
    ...
    def unpackItem(self,event,id):
        item = self.char.getItemById(str(id))
        self.char.unpackItem(item)
        self.updateItemList()

    # set the item as active bag
    def setActiveBag(self,event,id):
        self.active_bag_id = int(id)
        self.showEquippedItems()



    # open an item editor
    def displayItemEditor(self,event,id):
        item = self.char.getItemById(id)
