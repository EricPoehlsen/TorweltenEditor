...
class SocialEditor(tk.Toplevel):
    ...
    def showMenu(self,frame):
        ...
        delete = tk.Button(frame, text = msg.SE_DELETE, command = self.deleteContact)
    ...
    def showContact(self,frame):
        ...
        location_entry = tk.Entry(location_frame, textvariable = self.vars["location"])
        ...
        competency_entry = tk.Entry(competency_frame, textvariable = self.vars["competency"])
        competency_level = tk.Spinbox(competency_frame, from_ = 1, to = 9, textvariable = self.vars["competency_level"], width = 2)
        ...
        loyality_label = tk.Label(loyality_frame, textvariable = self.vars["loyality"])
        loyality_label.pack(fill = tk.X)
        loyality_level = tk.Scale(loyality_frame,from_=-3, to = 3,variable = self.vars["loyality_level"], showvalue = 0,orient = tk.HORIZONTAL)
        ...
        frequency_label = tk.Label(frequency_frame, textvariable = self.vars["frequency"])
        frequency_label.pack(fill = tk.X)
        frequency_level = tk.Scale(frequency_frame,from_=0, to = 5, variable = self.vars["frequency_level"], showvalue = 0,orient = tk.HORIZONTAL)
        ...
        description_text = tk.Text(description_frame,width = 40, height = 10, wrap = tk.WORD, font = "Arial 9")
        ...
        self.vars["name"].trace("w",self.dataChanged)
        self.vars["location"].trace("w",self.dataChanged)
        self.vars["competency"].trace("w",self.dataChanged)
        self.vars["competency_level"].trace("w",self.dataChanged)
        self.vars["loyality_level"].trace("w",self.dataChanged)
        self.vars["frequency_level"].trace("w",self.dataChanged)
        ...
        if self.vars["description"] == None: self.vars["description"] = ""
        description_text.insert(tk.END,self.vars["description"])
    
    # trace callback for most of the data ...
    def dataChanged(self,name,empty,mode):
        ...
        # frequency changed            
            ...
            if frequency_var == 0: self.vars["frequency"].set(msg.SE_FREQUENCY_0)
            elif frequency_var == 1: self.vars["frequency"].set(msg.SE_FREQUENCY_1)
            elif frequency_var == 2: self.vars["frequency"].set(msg.SE_FREQUENCY_2)
            elif frequency_var == 3: self.vars["frequency"].set(msg.SE_FREQUENCY_3)
            elif frequency_var == 4: self.vars["frequency"].set(msg.SE_FREQUENCY_4)
            elif frequency_var == 5: self.vars["frequency"].set(msg.SE_FREQUENCY_5)
    ...
    def wasChanged(self):
        ...
        if new_frequency_level != old_frequency_level: changed = True

        new_loyality_level = self.vars["loyality_level"].get()
        old_loyality_level = self.contact.get("loyality","")
        if new_loyality_level != old_loyality_level: changed = True
    ...
    def updateContact(self):
        ...
        if new_loyality == 0: new_loyality = 0.5
    ...
    def calculateCosts(self):
        ...
        if xp_cost_round == 0 and xp_cost > 0: xp_cost_round = 1
        if xp_cost_round == 0 and xp_cost < 0: xp_cost_round = -1
    ...
    def deleteContact(self):
        if self.delete_check:
            id = self.contact.get("id")
            
            # if generation mode, retun spent/gained xp
